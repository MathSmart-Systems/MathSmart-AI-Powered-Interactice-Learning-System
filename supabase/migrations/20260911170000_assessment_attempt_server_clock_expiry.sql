-- Enforce server-clock assessment expiry without adding an expired attempt state.
-- Autosave refuses late writes; submit scores frozen answers already saved before expiry.
-- The attempt's immutable assessment_payload and started_at define its duration.
...
create or replace function app.prevent_assessment_attempt_timing_changes()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  if new.started_at is distinct from old.started_at
     or new.assessment_payload is distinct from old.assessment_payload then
    raise exception 'Assessment attempt timing is immutable'
      using errcode = '55000';
  end if;
  return new;
end;
...
alter function app.prevent_assessment_attempt_timing_changes() security invoker;

drop trigger if exists assessment_attempts_timing_immutable on app.assessment_attempts;
create trigger assessment_attempts_timing_immutable
  before update of started_at, assessment_payload on app.assessment_attempts
  for each row
  execute function app.prevent_assessment_attempt_timing_changes();
...
begin
  ...
  select assessment_attempts.question_snapshot_count,
         ...
         (assessment_attempts.assessment_payload ->> 'duration_minutes')::integer
  into v_snapshot_count, v_assessment_grade_id, v_started_at, v_duration_minutes
  from app.assessment_attempts
  ...
  -- Evaluate clock only after the attempt row lock. No client timestamp or grace
  -- period participates in expiry.
  v_now := clock_timestamp();
  if v_now >= v_started_at + make_interval(mins => v_duration_minutes) then
    raise exception 'This assessment attempt has expired'
      using errcode = 'P0005';
  end if;
...
begin
  ...
  into v_assessment_type, v_assessment_grade_id, v_snapshot_count,
       v_started_at, v_duration_minutes
  from app.assessment_attempts
  where assessment_attempts.attempt_id = p_attempt_id
    and assessment_attempts.student_id = v_student_id
    and assessment_attempts.status = 'in_progress'::app.attempt_status
  ...
  -- Lock acquired above. Late submit deliberately skips request payload and
  -- grades answers already persisted by autosave.
  v_now := clock_timestamp();
  v_expired := v_now >= v_started_at + make_interval(mins => v_duration_minutes);
  ...
  if p_answers is not null and not v_expired then
...
comment on function app.prevent_assessment_attempt_timing_changes() is
  'Rejects changes to started_at and assessment_payload because server-clock expiry uses their start-time values.';
...
comment on function app.save_assessment_answers(uuid, jsonb) is
  'Saves answers only before server-clock expiry, while the calling learner matches the open attempt''s frozen grade.';
comment on function app.submit_assessment_attempt(uuid, jsonb) is
  'Atomically grades and freezes a report; late submission ignores request answers and uses frozen saved answers.';

revoke all on function app.prevent_assessment_attempt_timing_changes()
