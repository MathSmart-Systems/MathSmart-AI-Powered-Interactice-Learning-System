-- Transactional idempotency for final activity submissions.
-- Endpoint and actor are fixed so callers cannot cross operation or learner scope.

create function app.claim_activity_submission_idempotency(
...
begin
  if v_user_id is null
    ...
    raise exception 'Only an active learner may submit an activity'
      using errcode = '42501';
  end if;
  ...
  values (
    v_user_id,
    'POST /activity-attempts/{id}/submit',
    p_idempotency_key,
  ...
  where idempotency_keys.user_id = v_user_id
    and idempotency_keys.endpoint = 'POST /activity-attempts/{id}/submit'
...
create function app.complete_activity_submission_idempotency(
  p_idempotency_key text,
  p_request_fingerprint text,
...
begin
  ...
  if not found then
    raise exception 'No activity submission idempotency claim exists'
      using errcode = 'P0002';
  end if;
...
comment on function app.claim_activity_submission_idempotency(text, text) is
  'Claims an active learner activity submission key or replays its exact completed response.';

comment on function app.complete_activity_submission_idempotency(text, text, integer, jsonb) is
  'Completes an active learner activity submission claim with its exact HTTP response.';

revoke all on function app.claim_activity_submission_idempotency(text, text)
  from public, anon;
revoke all on function app.complete_activity_submission_idempotency(text, text, integer, jsonb)
  from public, anon;

grant execute on function app.claim_activity_submission_idempotency(text, text)
  to authenticated, service_role;
grant execute on function app.complete_activity_submission_idempotency(text, text, integer, jsonb)
  to authenticated, service_role;
