/**
 * The transport behind teacher interventions.
 *
 ...
 *
 * The assessments module carries its own copy of this contract. Both feature
 * services genuinely share the shape, so it is a candidate for promotion to
 * `src/modules/shared/` once a third caller appears; until then each owning
 * feature keeps its transport private per the module-ownership rules.
 */
...
export function createApiClient({
  ...
  async function request(path, { method = "GET", body = null, token = null } = {}) {
    ...
    // Archiving answers 204, which has no body to read at all.
